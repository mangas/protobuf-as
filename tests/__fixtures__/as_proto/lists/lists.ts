...
case 4: {
            ...
            obj.Messages.push(Message.decodeDataView(new DataView(decoder.view.buffer, decoder.pos+decoder.view.byteOffset, length)));
            decoder.skip(length);
...
break;
            ...
            if (this.Strings.length > 0) {
                for (let n:i32 = 0; n < this.Strings.length; n++) {
                    encoder.uint32(0x12); encoder.uint32(this.Strings[n].length); encoder.string(this.Strings[n])
                }
            }
            ...
            if (this.Bytes.length > 0) {
                for (let n:i32 = 0; n < this.Bytes.length; n++) {
                    encoder.uint32(0x1a); encoder.uint32(this.Bytes[n].length); encoder.bytes(this.Bytes[n])
                }
            }
