...
size += this.Value.length > 0 ? 1 + __proto.Sizer.varint64(this.Value.length) + this.Value.length : 0
...
if (this.Value.length > 0) { encoder.uint32(0xa); encoder.uint32(this.Value.length); encoder.string(this.Value) }
...
public StringExternalMapValue:Map<string, external.MapValue> = new Map<string, external.MapValue>();;
...
case 5: {
            ...
            const length = decoder.uint32();
            __decodeMap_string_external_MapValue(decoder, length, obj.StringExternalMapValue)
            decoder.skip(length);
...
break;
            ...
            if (this.StringInt32Map.size > 0) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (size > 0) {
                        ...
                        encoder.uint32(size)
                        if (key.length > 0) { encoder.uint32(0xa); encoder.uint32(key.length); encoder.string(key) }
                        if (value != 0) { encoder.uint32(0x10); encoder.int32(value) }
                    }
                }
            }
            ...
            if (this.Int32StringMap.size > 0) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (size > 0) {
                        encoder.uint32(0x1a)
                        encoder.uint32(size)
                        if (key != 0) { encoder.uint32(0x8); encoder.int32(key) }
                        if (value.length > 0) { encoder.uint32(0x12); encoder.uint32(value.length); encoder.string(value) }
                    }
                }
            }
            ...
            function __decodeMap_string_i32(parentDecoder: __proto.Decoder, length: i32, map: Map<string, i32>):void {
                const decoder = new __proto.Decoder(new DataView(parentDecoder.view.buffer, parentDecoder.pos+parentDecoder.view.byteOffset, length));
            ...
            function __decodeMap_i32_string(parentDecoder: __proto.Decoder, length: i32, map: Map<i32, string>):void {
                const decoder = new __proto.Decoder(new DataView(parentDecoder.view.buffer, parentDecoder.pos+parentDecoder.view.byteOffset, length));
            ...
            function __decodeMap_string_Value(parentDecoder: __proto.Decoder, length: i32, map: Map<string, Value>):void {
                const decoder = new __proto.Decoder(new DataView(parentDecoder.view.buffer, parentDecoder.pos+parentDecoder.view.byteOffset, length));
            ...
            const length = decoder.uint32();
            value = Value.decodeDataView(new DataView(decoder.view.buffer, decoder.pos+decoder.view.byteOffset, length))
            decoder.skip(length);
            ...
            function __decodeMap_string_external_MapValue(parentDecoder: __proto.Decoder, length: i32, map: Map<string, external.MapValue>):void {
                const decoder = new __proto.Decoder(new DataView(parentDecoder.view.buffer, parentDecoder.pos+parentDecoder.view.byteOffset, length));
            ...
            const length = decoder.uint32();
            value = external.MapValue.decodeDataView(new DataView(decoder.view.buffer, decoder.pos+decoder.view.byteOffset, length))
            decoder.skip(length);
                ...
                function __sizeMapEntry_i32_string(key: i32, value: string): u32 {
                    return (key == 0 ? 0 : 1 + __proto.Sizer.int32(key)) + 
                           (value.length > 0 ? 1 + __proto.Sizer.varint64(value.length) + value.length : 0);
                }
                ...
                function __sizeMapEntry_string_external_MapValue(key: string, value: external.MapValue): u32 {
                    const keySize = key.length > 0 ? 1 + __proto.Sizer.varint64(key.length) + key.length : 0;
                    const valueSize = value.size();
